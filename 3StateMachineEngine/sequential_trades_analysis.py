...
if __name__ == "__main__":
    ...
    # Diagnostics: show raw downloaded range vs usable post-indicator range.
    if df.empty:
        print("Raw data range   : n/a (no rows)")
    else:
        print(f"Raw data range   : {df.index.min().date()} -> {df.index.max().date()}  ({len(df)} rows)")
    if std_df.empty:
        print("Usable data range: n/a (warm-up/filters removed all rows)\n")
    else:
        print(f"Usable data range: {std_df.index.min().date()} -> {std_df.index.max().date()}  ({len(std_df)} rows)\n")
