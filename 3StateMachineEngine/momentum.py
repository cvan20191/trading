...
SMA_MID = 100
BAND_PCT = 0.005
# Momentum engine (replaces SMA regime when enabled)
USE_MOMENTUM_ENGINE = (
    True  # True => use absolute momentum regime; False => keep SMA regime
)
MOM_LOOKBACK_D = 252  # 252-trading-day momentum window
ABS_MOM_REF = (
    "SPY"  # SPYSIM reference for abs momentum filter (uses SPY adjusted close)
)
# If you want GLD/IEF momentum to also use 252 days, set:
# RISK_OFF_LOOKBACK_D = 252
# 3-state behavior
ONE_SIDED_2X = True
...
lineup_syms = set(
    RISK_ON_3X_LINEUP + RISK_ON_2X_LINEUP + RISK_OFF_GOLD_LINEUP + RISK_OFF_BOND_LINEUP
)
...
tickers = sorted(
    set([SIGNAL_SYMBOL, "^GSPC", "^NDX", "SPY", "GC=F"] + list(lineup_syms))
)
...
px = yf.download(
    tickers, start=dl_start, end=DATA_END, auto_adjust=False, progress=False
)
...
def get_adj_ohlc(px, sym, idx):
    ...
    ao = adj_open(
        px["Open"][sym].reindex(c.index), c, px["Adj Close"][sym].reindex(c.index)
    )
...
# Cash total-return index from rf_daily (CASHX)
cashx_index = (1.0 + rf_daily).cumprod().reindex(idx).ffill()
...
def _weekly_state_from_anchor(anchor="W-FRI"):
    ...
    if ONE_SIDED_2X:
        if ADAPTIVE_UP_WINDOW:
            ...
            up_mid_win_b = (
                up_mid.rolling(win_b, min_periods=1).max().fillna(0).astype(int)
            )
            up_mid_win_s = (
                up_mid.rolling(win_s, min_periods=1).max().fillna(0).astype(int)
            )
...
def _weekly_state_from_anchor_momentum(anchor="W-FRI"):
    """
    Absolute momentum regime:
      - Risk-on if SPYSIM 252d momentum > CASHX 252d momentum (both computed on daily data)
      - Baseline state: 2 (3x) when risk-on, else 0 (risk-off)
      - Then apply your existing risk controls:
          - Vol cap: degrade 2 -> 1 and 1 -> 0 when RV thresholds hit
          - DD throttle: degrade 2 -> 1 and 1 -> 0 when drawdown threshold hit
    """
    # Reference series (SPYSIM): prefer SPY adjusted close; fallback to ^GSPC if SPY missing
    ref_price = (
        spy_ac
        if ("SPY" in px["Adj Close"].columns and spy_ac.notna().any())
        else gspc_ac
    )

    # Daily 252d momentum
    spy_mom_d = ref_price / ref_price.shift(MOM_LOOKBACK_D) - 1.0
    cash_mom_d = cashx_index / cashx_index.shift(MOM_LOOKBACK_D) - 1.0

    # Align to weekly anchor (Thu/Fri)
    spy_mom_w = spy_mom_d.resample(anchor).last()
    cash_mom_w = cash_mom_d.resample(anchor).last()

    # Objective risk measures (same as your SMA engine)
    ndx_ret = ndx_ac.pct_change()
    rv_d = ndx_ret.rolling(VOL_LOOKBACK_D).std() * math.sqrt(252)
    rv_w = rv_d.resample(anchor).last().reindex(spy_mom_w.index).ffill()

    running_max_ndx = ndx_ac.rolling(
        DD_LOOKBACK_D, min_periods=max(1, int(DD_LOOKBACK_D * 0.8))
    ).max()
    dd_d = ndx_ac / running_max_ndx - 1.0
    dd_w = dd_d.resample(anchor).last().reindex(spy_mom_w.index).ffill()

    # Baseline state: 2 if SPY_mom > CASH_mom, else 0
    state_w = pd.Series(0, index=spy_mom_w.index, dtype=int)
    risk_on = (spy_mom_w > cash_mom_w) & spy_mom_w.notna() & cash_mom_w.notna()
    state_w[risk_on] = 2

    # Apply your existing caps/throttles
    if USE_VOL_CAP:
        state_w[(state_w == 2) & (rv_w > VOL_TH_3X)] = 1
        state_w[(state_w >= 1) & (rv_w > VOL_TH_ROFF)] = 0
...
def rebuild_regime():
    ...
    # Ensure ref_ac uses current SIGNAL_SYMBOL (kept for warm-up/gating elsewhere)
    if (
    ...
    # Keep SMA_slow for any warm-up masks; regime engine may ignore it
    sma_slow = ref_ac.rolling(SMA_SLOW, min_periods=SMA_SLOW).mean()

    # Weekly states anchored to Thursday and Friday
    if USE_MOMENTUM_ENGINE:
        state_thu = _weekly_state_from_anchor_momentum("W-THU")
        state_fri = _weekly_state_from_anchor_momentum("W-FRI")
    else:
...
def build_locked_schedule(run_idx):
    ...
    def has_data(sym, d):
        ...
        return pd.notna(
            ac_extra.get(sym, pd.Series(index=idx, dtype=float)).get(d, np.nan)
        )
    ...
    def choose_risk_off(d):
        if USE_DUAL_RISK_OFF and HAVE_IEF:
            return (
                "GLD"
                if float(mom_gld_use.get(d, -1.0)) >= float(mom_ief_use.get(d, -1.0))
                else "IEF"
            )
        return "GLD"
    ...
    lockout_until = pd.Timestamp(
        "1900-01-01"
    )  # Nasdaq-family lock (your original global)
    curr_asset = None
    entry_date = None
    leg_gross = 1.0
    ...
    for i, d in enumerate(run_idx):
        ...
        if i == 0:
            ...
            nxt = pick_available_from_lineup(
                base, regime, d, lockout_until, prev_asset=None
            )
            if nxt is None:
                # last resort: scan any lineup name with data
                any_syms = (
                    list(RISK_ON_3X_LINEUP)
                    + list(RISK_ON_2X_LINEUP)
                    + list(RISK_OFF_GOLD_LINEUP)
                    + list(RISK_OFF_BOND_LINEUP)
                )
        ...
        if d in swap_set:
            ...
            next_asset = pick_available_from_lineup(
                base, regime, d, lockout_until, prev_asset=curr_asset
            )
...
def trade_cost(d):
    bps = (
        SLIPPAGE_BPS_STRESS
        if (d in ref_ret_abs.index and ref_ret_abs.loc[d] >= STRESS_THRESHOLD)
        else SLIPPAGE_BPS
    )
    return bps / 10000.0
...
def sim_strategy(run_idx, paper=False):
    ...
    for i, d in enumerate(run_idx):
        ...
        div_amt = float(
            div_series.get(curr_asset, pd.Series(0.0, index=run_idx)).loc[d]
        )
...
def build_regime_by_SMA_baseline_dual():
    ...
    def baseline_state(anchor):
        ...
        for dt in ref_w.index:
            ...
            dn = (1 - BAND_PCT) * m
            curr = (
                1 if (prev == 0 and p > up) else (0 if (prev == 1 and p < dn) else prev)
            )
            reg_w.loc[dt] = curr
            prev = curr
...
def run_pure_baseline_segment(name, start, end):
    ...
    saved_swap, saved_reg = (
        globals().get("swap_dates_all", None),
...
def walk_forward_validation(
    oos_start, oos_end, train_years, test_years, paper=True, chain_capital=True
):
...
def weekly_signal_dashboard(window_start="2019-01-01", lookback_legs=12, now_ts=None):
    ...
    has_today_bar = idx[-1].normalize() == pd.Timestamp.now().normalize()
    ...
    print(f"Today (market calendar): {today.date()}")

    if not (made_thu or made_fri):
        print("Decision status: NO – refresh after Thursday/Friday close.")
        ...
        print("Next decision windows:")
        if next_thu is not None:
            next_thu_exec = next_trading_day(next_thu)
            exec_txt = next_thu_exec.date() if next_thu_exec is not None else "n/a"
            print(f"  Thu {next_thu.date()} close → {exec_txt} open")
        if next_fri is not None:
            next_fri_exec = next_trading_day(next_fri)
            exec_txt = next_fri_exec.date() if next_fri_exec is not None else "n/a"
            print(f"  Fri {next_fri.date()} close → {exec_txt} open")
        ...
        print("Last trade made on: n/a in window")
        print(f"Currently holding: {curr if curr else 'n/a'}")
        print("==============================================")
        return
    ...
    # If we're at the end of the calendar, fall back to the most recent trading day
    exec_fallback = False
    if exec_day is None:
        exec_day = idx[-1]
        exec_fallback = True
    ...
    fallback_txt = " [calendar fallback]" if exec_fallback else ""
    print(
        f"Execution: {exec_day.date()} open{fallback_txt} | Suggested: {suggested if suggested else 'TBD'}"
    )
    ...
    if next_decision is not None and next_exec is not None:
        print(
            f"Next decision window: {next_decision.date()} close → {next_exec.date()} open"
        )
    else:
    ...
    print(f"Last trade made on: {last_trade_date}")
    latest_holding = sched_hist.iloc[-1] if not sched_hist.empty else None
    print(f"Currently holding: {latest_holding if latest_holding else 'n/a'}")
    print("==============================================")
...
rebuild_regime()
run_idx_chk = build_run_index("2000-01-01", None)
sig_chk = regime_intraday_all.reindex(run_idx_chk)
print("Risk-on share (days):", float((sig_chk == 2).mean()))

if __name__ == "__main__":
    # First two tweaks only: momentum regime + 252d window
    USE_MOMENTUM_ENGINE = True
    MOM_LOOKBACK_D = 252
    HOLDOUT = ("2010-01-01", "2025-12-31")

    rebuild_regime()  # build SPY vs CASHX absolute momentum states
    cp_hold, cr_hold, m_hold = run_segment(
        "Holdout (Momentum 252d)", HOLDOUT[0], HOLDOUT[1]
    )
    if m_hold:
        print_metrics("Holdout (Paper)", m_hold[0])
        print_metrics("Holdout (Real)", m_hold[1])

    # Sanity: fraction of risk-on days
    run_idx_chk = build_run_index(HOLDOUT[0], HOLDOUT[1])
    sig_chk = regime_intraday_all.reindex(run_idx_chk)
    print("Risk-on share (days):", float((sig_chk == 2).mean()))
