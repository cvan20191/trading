...
OOS_MAIN          = ("1999-01-01", "2026-03-20")
HOLDOUT           = ("2025-10-28",None)

START_CAPITAL   = 27000.0
...
RISK_ON_2X_LINEUP    = ["QLD",  "SSO", "DDM",  "UWM",  "ROM"]
...
def build_locked_schedule(run_idx):
    ...
    # Per-symbol wash-sale lockouts: prevent same-ticker repurchase within 30d after loss
    lockouts_psym = defaultdict(lambda: pd.Timestamp("1900-01-01"))
    def is_locked_sym(sym, d):
        return d <= lockouts_psym.get(sym, pd.Timestamp("1900-01-01"))

    # Availability + lockout-aware picker (keeps your priority/lineups)
    def pick_available_from_lineup(base, regime, d, lock_until, prev_asset):
        if regime == 2:
        ...
        # First pass: obey per-symbol locks and data
        for s in candidates:
            if is_locked_sym(s, d):
                continue
            if has_data(s, d):
                return s

        # If everything viable is locked/unavailable, return prev_asset to signal "no trade"
        return prev_asset

    sched = pd.Series(index=run_idx, dtype=object)
    lockout_until = pd.Timestamp("1900-01-01")  # Nasdaq-family lock (your original global)
    curr_asset = None
    entry_date = None
    leg_gross = 1.0
    ...
    for i, d in enumerate(run_idx):
        ...
        if i == 0:
            ...
            nxt = pick_available_from_lineup(base, regime, d, lockout_until, prev_asset=None)
            if nxt is None:
                # last resort: scan any lineup name with data
                any_syms = list(RISK_ON_3X_LINEUP) + list(RISK_ON_2X_LINEUP) + list(RISK_OFF_GOLD_LINEUP) + list(RISK_OFF_BOND_LINEUP)
                nxt = next((s for s in any_syms if has_data(s, d)), None)
            curr_asset = nxt if nxt is not None else base
            entry_date = d
        ...
        if d in swap_set:
            base = choose_asset(regime, d, lockout_until)
            next_asset = pick_available_from_lineup(base, regime, d, lockout_until, prev_asset=curr_asset)

            if next_asset == curr_asset or next_asset is None:
                # No viable change (or best equals current): NO TRADE => no sale => no wash risk
                leg_gross *= (1.0 + gap_map[curr_asset].get(d, 0.0)) * (1.0 + intra_map[curr_asset].get(d, 0.0))
                sched.iloc[i] = curr_asset
                continue

            # We will switch: sell current at open, then buy next at open
            leg_gross *= (1.0 + gap_map[curr_asset].get(d, 0.0))

            # If the closed leg is a loss, set:
            # - your original Nasdaq-family lock (TQQQ/QLD)
            # - per-symbol 30d lock to prevent same-ticker repurchase (for-sure wash avoidance)
            if (leg_gross - 1.0) < 0.0:
                if curr_asset in {"TQQQ", "QLD"}:
                    lockout_until = d + Day(LOCKOUT_DAYS)
                lockouts_psym[curr_asset] = d + Day(LOCKOUT_DAYS)
...
def _get_nasdaq_lockout_from_legs(completed_legs):
    """
    Re-derive the Nasdaq-family lockout expiry from completed leg history.
    A lockout is triggered when TQQQ or QLD is closed at a net loss.
    """
    lockout_until = pd.Timestamp("1900-01-01")
    if completed_legs is None or completed_legs.empty:
        return lockout_until
    for _, leg in completed_legs.iterrows():
        if leg["Asset"] in {"TQQQ", "QLD"} and float(leg["ReturnPct"]) < 0.0:
            candidate = pd.Timestamp(leg["Exit"]) + Day(LOCKOUT_DAYS)
            if candidate > lockout_until:
                lockout_until = candidate
    return lockout_until

def _regime_to_asset(regime, exec_dt, lockout_until):
    """
    Map a 3-state regime integer to the preferred asset symbol,
    respecting the Nasdaq-family lockout and dual risk-off momentum.
    Mirrors the choose_asset() logic inside build_locked_schedule.
    """
    if regime == 2:
        return "SPXL" if exec_dt <= lockout_until else "TQQQ"
    elif regime == 1:
        return "SSO"  if exec_dt <= lockout_until else "QLD"
    else:                                      # regime == 0: risk-off
        if USE_DUAL_RISK_OFF and globals().get("HAVE_IEF", False):
            g  = float(mom_gld_use.get(exec_dt, -1.0))                  if exec_dt in mom_gld_use.index else -1.0
            iv = float(mom_ief_use.get(exec_dt, -1.0))                  if exec_dt in mom_ief_use.index else -1.0
            return "GLD" if g >= iv else "IEF"
        return "GLD"
...
def weekly_signal_dashboard(window_start="2019-01-01", lookback_legs=12, now_ts=None):
    ...
    # -- 1. Resolve "today" ------------------------------------------------
    if now_ts is None:
        real_now = pd.Timestamp.now(tz=None).normalize()
        if real_now not in idx:
            pos = idx.searchsorted(real_now, side="right") - 1
            today = idx[max(0, pos)]
        else:
            today = real_now
        if today > idx[-1]:
        ...
        today = pd.Timestamp(now_ts).normalize()
        if today not in idx:
            pos = idx.searchsorted(today, side="right") - 1
            today = idx[max(0, pos)]

    has_today_bar = (
        idx[-1].normalize() == pd.Timestamp.now(tz=None).normalize()
    )

    # -- 2. Decision detection (weekday-based) -----------------------------
    made_thu = (today.weekday() == 3)   # Thursday = 3
    made_fri = (today.weekday() == 4)   # Friday   = 4

    # -- 3. Build historical schedule for context / lock propagation --------
    run_idx_hist = idx[
        (idx >= pd.to_datetime(window_start)) & (idx <= today)
    ]
    sched_hist, legs_hist = build_schedule_and_legs(run_idx_hist)
    curr_holding = sched_hist.iloc[-1] if not sched_hist.empty else None

    print("\n===== Weekly Execution Dashboard =====")
    print(
        f"Today (market calendar): {today.date()}"
        f"  [{today.day_name()}]"
    )

    # -- 4. NO-DECISION branch ---------------------------------------------
    if not (made_thu or made_fri):
        REGIME_LABEL = {
            0: "Risk-Off   (State 0 → GLD / IEF)",
            1: "2× Lev.    (State 1 → QLD / SSO)",
            2: "3× Lev.    (State 2 → TQQQ / SPXL)",
        }
        current_regime = int(regime_intraday_all.get(today, regime_intraday_all.iloc[-1]))
        regime_lbl = REGIME_LABEL.get(current_regime, f"State {current_regime}")

        open_leg_ret_str = "n/a"
        open_leg_entry = "n/a"
        open_leg_days = 0
        if legs_hist is not None and not legs_hist.empty and curr_holding:
            last_exit = pd.Timestamp(legs_hist.iloc[-1]["Exit"])
            open_leg_entry = last_exit
            open_leg_mask = (run_idx_hist >= last_exit) & (run_idx_hist <= today)
            open_idx = run_idx_hist[open_leg_mask]
            open_leg_days = len(open_idx)
            g_map = {
                "TQQQ": gap_TQQQ, "QLD": gap_QLD,
                "SPXL": gap_SPXL, "SSO": gap_SSO, "GLD": gap_GLD
            }
            i_map = {
                "TQQQ": intra_TQQQ, "QLD": intra_QLD,
                "SPXL": intra_SPXL, "SSO": intra_SSO, "GLD": intra_GLD
            }
            if globals().get("HAVE_IEF", False):
                g_map["IEF"] = gap_IEF; i_map["IEF"] = intra_IEF
            if curr_holding in g_map and len(open_idx) > 0:
                gross = (1.0 + float(i_map[curr_holding].get(open_idx[0], 0.0)))
                for od in open_idx[1:]:
                    gross *= (
                        (1.0 + float(g_map[curr_holding].get(od, 0.0)))
                        * (1.0 + float(i_map[curr_holding].get(od, 0.0)))
                    )
                open_leg_ret = (gross - 1.0) * 100.0
                open_leg_ret_str = f"{open_leg_ret:+.2f}%"

        last_trade_str = "none"
        if legs_hist is not None and not legs_hist.empty:
            lt = legs_hist.iloc[-1]
            lt_ret = float(lt["ReturnPct"]) * 100.0
            exit_dt = pd.Timestamp(lt['Exit']).date()
            last_trade_str = (f"{lt['Asset']} → {lt['Next']}  on {exit_dt}  "
                              f"({lt_ret:+.2f}%, {int(lt['Days'])}d)")

        # Next decision windows
        pos = idx.searchsorted(today, side="right")
        next_thu = next_fri = None
        while pos < len(idx) and (next_thu is None or next_fri is None):
            d = idx[pos]
            if d.weekday() == 3 and next_thu is None:
                next_thu = d
            if d.weekday() == 4 and next_fri is None:
                next_fri = d
            pos += 1

        print("\nNext decision windows:")
        if next_thu is not None:
            nte = next_trading_day(next_thu)
            print(f"  Thu {next_thu.date()} close  →  "
                  f"{nte.date() if nte else 'n/a'} open")
        if next_fri is not None:
            nfe = next_trading_day(next_fri)
            print(f"  Fri {next_fri.date()} close  →  "
                  f"{nfe.date() if nfe else 'n/a'} open")
        if next_thu is None and next_fri is None:
            print("  None remaining in calendar.")
        ...
        print(f"\nHas today's bar?  {has_today_bar}")

        SEP = "=" * 56
        entry_dt = pd.Timestamp(open_leg_entry).date() if open_leg_entry != "n/a" else "n/a"
        print(f"\n{SEP}")
        print(f"  HOLDING  —  no decision until Thu/Fri close")
        print(f"{SEP}")
        print(f"  Asset    :  {curr_holding if curr_holding else 'n/a'}")
        print(f"  Regime   :  {current_regime}  ({regime_lbl})")
        print(f"  Open leg :  {entry_dt} → now  "
              f"({open_leg_days}d, {open_leg_ret_str})")
        print(f"  Last trade: {last_trade_str}")
        print(f"{SEP}")
        return
    ...
    exec_day_in_idx = next_trading_day(today)   # None when today == idx[-1]
    exec_fallback   = (exec_day_in_idx is None)
    exec_day        = (
        exec_day_in_idx if not exec_fallback
        else (today + BDay(1)).normalize()
    )

    # -- 5a. Derive the suggested asset ------------------------------------
    if not exec_fallback:
        run_idx_until_exec = idx[idx <= exec_day]
        sched_upto, _ = build_schedule_and_legs(run_idx_until_exec)
        suggested  = (
            sched_upto.loc[exec_day]
            if exec_day in sched_upto.index else None
        )
        next_regime = int(regime_intraday_all.get(exec_day, 0))

    else:
        # THE CORE FIX: exec_day (Monday) is beyond idx.
        if made_fri and len(state_fri) > 0:
            next_regime = int(state_fri.iloc[-1])
        elif made_thu and len(state_thu) > 0:
            next_regime = int(state_thu.iloc[-1])
        else:
            next_regime = int(regime_intraday_all.iloc[-1])

        lockout_until = _get_nasdaq_lockout_from_legs(legs_hist)

        # If the open leg being closed is TQQQ/QLD at a loss, fold that
        # lockout in now (the trade hasn't happened yet but we are deciding).
        if curr_holding in {"TQQQ", "QLD"}:
            entry_leg = None
            if legs_hist is not None and not legs_hist.empty:
                last_exit = legs_hist.iloc[-1]["Exit"]
                entry_leg = last_exit
            if entry_leg is not None:
                open_leg_mask = (run_idx_hist >= entry_leg)
                open_idx = run_idx_hist[open_leg_mask]
                g_map = {
                    "TQQQ": gap_TQQQ, "QLD": gap_QLD,
                    "SPXL": gap_SPXL, "SSO": gap_SSO, "GLD": gap_GLD
                ...
                if curr_holding in g_map and len(open_idx) > 0:
                    gross = (
                        (1.0 + float(i_map[curr_holding].get(open_idx[0], 0.0)))
                    )
                    for od in open_idx[1:]:
                        gross *= (
                            (1.0 + float(g_map[curr_holding].get(od, 0.0)))
                    ...
                    if (gross - 1.0) < 0.0:
                        candidate_lock = today + Day(LOCKOUT_DAYS)
                        if candidate_lock > lockout_until:
                            lockout_until = candidate_lock

        suggested = _regime_to_asset(next_regime, exec_day, lockout_until)

    action_required = (
        suggested is not None and suggested != curr_holding
    )

    # -- 5b. Print decision summary ----------------------------------------
    REGIME_LABEL = {
        0: "Risk-Off   (State 0 → GLD / IEF)",
        1: "2× Lev.    (State 1 → QLD / SSO)",
        2: "3× Lev.    (State 2 → TQQQ / SPXL)",
    }
    regime_lbl = REGIME_LABEL.get(next_regime, f"State {next_regime}")
    fallback_note = (
        "  ⚠  hypothetical date — beyond available data"
        if exec_fallback else ""
    )

    print(f"Decision status : YES — {anchor} close {today.date()}")
    print(f"Execution day   : {exec_day.date()}{fallback_note}")
    print(f"Next regime     : {next_regime}  ({regime_lbl})")

    # -- 5c. Next decision window ------------------------------------------
    nxt_wd = 4 if made_thu else 3
    next_decision = _find_next_trading_day_with_weekday(today, nxt_wd)
    if next_decision is not None:
        nxt_exec = next_trading_day(next_decision)
        exec_str = nxt_exec.date() if nxt_exec is not None else "n/a"
        print(f"Next decision window: {next_decision.date()} close  →  "
              f"{exec_str} open")
    else:
        print("Next decision window: none remaining in calendar.")

    # -- 5d. Recent completed legs -----------------------------------------
    if legs_hist is not None and not legs_hist.empty:
        legs_tail = legs_hist.tail(lookback_legs).copy()
        legs_tail["Return%"] = (legs_tail["ReturnPct"] * 100.0).round(2)
        cols = ["Entry", "Exit", "Asset", "Next", "Days", "Return%"]
        print("\nRecent completed legs (open→open):")
        print(legs_tail[cols].to_string(index=False))
        last_trade_date = legs_tail.iloc[-1]["Exit"]
    else:
        print("\nNo completed legs in window.")
        last_trade_date = "n/a"
    ...
    print(f"\nHas today's bar?  {has_today_bar}")
    print("Repainting : NO  (decision fixed at Thu/Fri close; "
          "executed next trading-day open).")
    print("Lag        : 1 BD by design  "
          "(Thu→Fri open  |  Fri→Mon open).")
    print(f"Last completed leg : {last_trade_date}")
    print(f"Currently holding  : {curr_holding if curr_holding else 'n/a'}")
    print(f"Open leg           : {entry_dt} → now  "
          f"({open_leg_days}d, {open_leg_ret_str})")
    ...
    if action_required:
        print(f"\n{SEP}")
        print(f"  ⚡  ACTION REQUIRED  —  trade at MARKET OPEN")
        print(f"  SELL  :  {curr_holding}")
        print(f"  BUY   :  {suggested}")
        print(f"  WHEN  :  {exec_day.date()} market open")
        print(f"{SEP}")
    else:
        current_regime = int(regime_intraday_all.iloc[-1])
        print(f"\n✅  NO TRADE  —  hold {suggested} through next open")
        print(f"   Regime unchanged : {current_regime} → {next_regime}")
        print(f"   Currently holding: {curr_holding}")
    print("==============================================")
...
if not DO_WALK_FORWARD:
    ...
    weekly_signal_dashboard(window_start="2019-01-01", lookback_legs=50)
